...
const server = http.createServer(async (req, res) => {
    ...
    // API endpoint pour initialiser avec un port série
    if (path === '/api/init' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                const { serial_port } = data;

                if (!serial_port) {
                    res.writeHead(400, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({
                        status: 'error',
                        error: 'Paramètre manquant: serial_port est requis'
                    }));
                    return;
                }

                await initializeRFXCOM(serial_port);
