...
- **Correction du problème avec AUTO_DISCOVERY activé** :
  - Correction du problème où RFXCOM ne fonctionnait plus quand `auto_discovery` était activé
  - Amélioration du fallback pour enregistrer les listeners même si `receiverstarted` n'est pas émis
  - Les commandes fonctionnent maintenant correctement avec `auto_discovery: true`
  - Logs de diagnostic améliorés pour identifier les problèmes d'initialisation

### Version 2.1.8
- **Vérification que RFXCOM est prêt avant d'envoyer des commandes** :
  - Les commandes sont maintenant bloquées si RFXCOM n'est pas complètement prêt
  - Attente de l'événement `receiverstarted` avant d'accepter les commandes
  - Fallback automatique après 5 secondes si l'événement n'est pas émis
  - Messages d'erreur clairs si une commande est envoyée trop tôt
...
- **Fallback pour l'initialisation RFXCOM** :
  - Si l'événement `ready` est émis mais que le callback `initialise` n'est pas appelé, l'add-on continue quand même l'initialisation
  - Cela corrige le problème où l'add-on attendait indéfiniment le callback malgré une connexion réussie
  - Détection améliorée des échecs de connexion avec le listener `connectfailed`

### Version 2.1.5
- **Correction majeure de l'initialisation RFXCOM** :
  - Nettoyage automatique de l'instance RFXCOM précédente avant création d'une nouvelle
  - Réorganisation de l'ordre d'enregistrement des listeners (avant `initialise()`)
  - Correction du problème où le callback `initialise` n'était jamais appelé, causant un timeout systématique
  - Les listeners sont maintenant enregistrés dans le bon ordre pour ne pas manquer les événements
