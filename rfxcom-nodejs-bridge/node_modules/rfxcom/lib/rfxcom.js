...
class RfxCom extends EventEmitter {
        ...
        // Allow for faking out the SerialPort (e.g. in tests; port.device is set so fake port can simulate ACK)
        if (typeof this.options.port !== "undefined") {
            this.serialport = options.port;
            this.serialport.device = this;
        }
        ...
        this.TxQ.on("timeout", function (next, transmission) {
            const seqnbr = transmission.seqnbr;
            const userCb = self.pendingCallbacks[seqnbr];
            if (typeof userCb === "function") {
                self.pendingCallbacks[seqnbr] = null;
                const err = new Error("Command message " + RfxCom.dumpHex([seqnbr]) + ", timed out waiting for response");
                try { userCb(err, null, seqnbr); } catch (e) { /* ignore */ }
            }
            if (transmission.sender._timeoutHandler.call(transmission.sender, transmission.buffer, seqnbr) === false) {
                self.debugLog("Error   : Command message " + RfxCom.dumpHex([seqnbr]) +
                    ", timed out waiting for response");
                self.emit("response", "Timed out waiting for response", seqnbr, rfxcom.responseCode.TIMEOUT);
            }
            next();
        });
        // Holds the command acknowledgement callbacks for the transmit queue (queue's next())
        this.acknowledge = new Array(256);
        for (let idx = 0; idx < this.acknowledge.length; idx++) {
            this.acknowledge[idx] = null;
        }
        // User callbacks for queued messages: called on ACK or timeout (not on write)
        this.pendingCallbacks = new Array(256);
        for (let idx = 0; idx < this.pendingCallbacks.length; idx++) {
            this.pendingCallbacks[idx] = null;
        }
    ...
    /**
     * For testing: simulate receipt of ACK for a given seqnbr (invokes user callback and queue next).
     * @param {number} seqnbr - sequence number
     */
    simulateAck(seqnbr) {
        const userCb = this.pendingCallbacks[seqnbr];
        if (typeof userCb === "function") {
            this.pendingCallbacks[seqnbr] = null;
            try { userCb(null, "ACK - transmit OK", seqnbr); } catch (e) { /* ignore */ }
        }
    ...
    queueMessage(sender, buffer, seqnbr, callback) {
        ...
        if (self.connected) {
            ...
            // Store user callback to invoke on ACK or timeout (not on write)
            self.pendingCallbacks[seqnbr] = callback || null;
            self.TxQ.push(function () {
                    let transmission = function (cb) {
                        self.acknowledge[seqnbr] = cb;
                        self.transmit(buffer, seqnbr, null); // do not call user callback on write
                    };
...
//------------------------------------------
    ...
    statusMessageHandler(data) {
        ...
        // Détecter les sondes Alecto TH13/WS1700 qui utilisent packet type 0x01 avec un format spécial
        // Le paquet se termine par "RFXCOM" mais contient les données de la sonde avant
        // Format Node.js: [subtype=0x00, seqnbr, channel, id_byte1, id_byte2, temp_integer, temp_fraction, ...]
        // Température: temp_integer + (temp_fraction / 256)
        // Humidité: temp_fraction utilisé aussi pour l'humidité avec facteur de conversion
        if (subtype === 0x00 && data.length >= 19) {
            // Vérifier si c'est un message de sonde Alecto (contient "RFXCOM" à la fin)
            const endText = String.fromCharCode.apply(String, data.slice(data.length - 6));
            if (endText === "RFXCOM" || endText === "XCOM") {
                // C'est une sonde Alecto TH13/WS1700
                // Format: data[0]=subtype(0), data[1]=seqnbr, data[2]=channel, data[3-4]=ID, data[4-5]=temp, data[5]=humidity_raw
                const sequence = data[1];
                const channel = data[2];
                const sensorId = "0x" + RfxCom.dumpHex(data.slice(3, 5), false).join("");
                
                // Température: octet entier + octet fractionnaire / 256
                // data[4] = température partie entière (ex: 21)
                // data[5] = température partie fractionnaire (ex: 127)
                const tempInteger = data[4];
                const tempFraction = data[5];
                const temperature = tempInteger + (tempFraction / 256);
                
                // Humidité: utilise le même octet que la fraction de température avec facteur de conversion
                // Facteur empirique: raw * 100 / 327
                const humidityRaw = data[5] & 0x7F;
                const humidity = Math.round(humidityRaw * 100 / 327);
                
                // Status, signal, battery (positions à vérifier selon le format exact)
                const status = data[7] || 0;
                const signalRaw = data[8] || 0;
                const batteryLevel = (data[9] & 0x0f) || (signalRaw & 0x0f);
                const rssi = ((data[9] >> 4) & 0xf) || ((signalRaw >> 4) & 0xf);
                
                const evt = {
                    subtype: 13, // TH13 (0x0D)
                    id: sensorId,
                    seqnbr: sequence,
                    channel: channel,
                ...
                // Émettre l'événement temperaturehumidity1 (compatible avec le format standard)
                self.emit("temperaturehumidity1", evt, 0x01);
                self.debugLog("Alecto TH13/WS1700 sensor detected: ID=" + sensorId + ", temp=" + evt.temperature + "°C, hum=" + evt.humidity + "%");
                return; // Ne pas traiter comme un message de statut normal
            }
        }
    ...
    transmitCommandResponseHandler(data) {
        ...
        if (typeof userCb === "function") {
            this.pendingCallbacks[seqnbr] = null;
            const err = (message === 0 || message === 1) ? null : new Error(responses[message] || "Transmit failed");
            const response = responses[message];
            try { userCb(err, response, seqnbr); } catch (e) { /* ignore */ }
        }
        if (this.acknowledge[seqnbr] !== undefined && typeof this.acknowledge[seqnbr] === "function") {
            this.acknowledge[seqnbr]();
            this.acknowledge[seqnbr] = null;
 ...
 */
    blinds1Handler(data, packetType) {
        ...
        if (evt.subtype === 0x00 || evt.subtype === 0x01 || evt.subtype === 0x0c ||
            ...
            evt.subtype === 0x13 || evt.subtype === 0x14) {
            evt.id = "0x" + RfxCom.dumpHex(data.slice(3, 5), false).join("");
        ...
        if (evt.subtype === 0x02 || evt.subtype === 0x04 || evt.subtype === 0x05 ||
            evt.subtype === 0x0a || evt.subtype === 0x12) {
            evt.unitCode = 1;
        } else {
            evt.unitCode = data[5];
...
/*
 *
 * Called by the data event handler when data arrives from Honeywell ActivLink devices
 * (packet type 0x1d)
 *
 */
activLinkHandler(data, packetType) {
    const
        subtype = data[0],
        evt = {
            subtype: subtype,
    ...
    if (subtype === 0x00) {
        evt.commandNumber = data[6];
        evt.command = rfxcom.commandName(packetType, subtype, data[6]);
    } else if (subtype === 0x01) {
        evt.deviceStatus = data[6];
    }
    this.emit("activlink", evt, packetType);
    };
/*
 *
    ...
    hunterfanHandler(data, packetType) {
        const
            ...
            evt = {
                ...
                command: rfxcom.commandName(packetType, subtype, data[8]),
                rssi: (data[9] >> 4) & 0xf
            };
    ...
    camera1Handler(data, packetType) {
        const
            ...
            evt = {
                    ...
                    command: rfxcom.commandName(packetType, subtype, data[3]),
                    rssi: (data[5] >> 4) & 0xf
                };
...
/*
 *
 * Called by the data event handler when data arrives from a blinds2 controller (packet type 0x31).
 *
 */
    blinds2Handler(data, packetType) {
        const
        subtype = data[0],
        seqnbr = data[1],
        evt = {
            ...
            percent: data[8],
            angle: data[9],
            batteryLevel: data[10] & 0x0f,
            rssi: (data[10] >> 4) & 0xf
        };
        if (data[6] === 0x10) {
            evt.unitCode = 0;
        } else {
            evt.unitCode = data[6] + 1;
        }
        this.emit("blinds2", evt, packetType);
};
    ...
    thermostat3Handler(data, packetType) {
        const
            ...
            evt = {
                ...
                id: "0x" + RfxCom.dumpHex(data.slice(2, 5), false).join(""),
                commandNumber: data[5],
                command: rfxcom.commandName(packetType, subtype, data[5]),
                rssi: (data[6] >> 4) & 0xf
            };
 ...
 */
    rainHandler(data, packetType) {
        ...
        if (data[0] === 6) {  //RAIN6
            ...
            evt.rainfallIncrement = ((data[7])*256 + data[8])*0.254;
        } else if (data[0] === 10) { //RAIN10
            evt.rainfallIncrement = ((data[7])*256 + data[8])*0.1;
        } else {
 ...
 */
    ...
    uvHandler(data, packetType) {
        ...
        if (evt.subtype === 0x03) {
            const signbit = data[5] & 0x80;
            evt.temperature = ((data[5] & 0x7f)*256 + data[6])/10 * (signbit ? -1 : 1);
        }
...
/*
 *
 * Called by the data event handler when data arrives from a TS FT002 water level sensor (packet type 0x73).
 *
 */
    waterlevelHandler(data, packetType) {
        let evt = {
            subtype: data[0],
            seqnbr: data[1],
            id: "0x" + RfxCom.dumpHex(data.slice(2, 4), false).join(""),
        ...
        let temperature = ((data[7] & 0x7f)*256 + data[8])/10,
            signbit = data[7] & 0x80;
        evt.temperature = temperature*(signbit ? -1 : 1);
        evt.level = data[5]*256 + data[6];
        this.emit("waterlevel", evt, packetType);
    };
/*
 *
 * Called by the data event handler when data arrives from an EcoWitt WH57 lightning sensor (packet type 0x74).
 *
 */
    lightningHandler(data, packetType) {
        let evt = {
            subtype: data[0],
            seqnbr: data[1],
            id: "0x" + RfxCom.dumpHex(data.slice(2, 5), false).join(""),
            status: {0: "start-up", 1: "interference", 4: "noise", 8: "strike"}[data[5]],
            distance: data[6],
            strikes: data[7],
            batteryLevel: data[10] & 0x0f,
            rssi: (data[10] >> 4) & 0x0f
        };
        evt.valid = evt.distance != 63;
        this.emit("lightning", evt, packetType);
    };
/*
 *
 ...
 */
    weatherHandler(data, packetType) {
        ...
        if (evt.subtype === 0) {
            evt.rainfallIncrement = (((data[18])*256 + data[19])*256 + data[20])*0.1;
            evt.direction = data[4]*256 + data[5];
            evt.humidity = data[14];
            evt.humidityStatus = data[15];
            evt.uv = data[21]/10;
            evt.insolation = data[22]*256 + data[23];
            evt.batteryLevel = data[29] & 0x0f;
        } else if (evt.subtype === 1) {
    ...
    static list(callback) {
        if (callback && typeof callback === "function") {
            SerialPort.list().then((ports) => {
                const rfxcomPorts = ports.filter(port => {return /RFXCOM|Silicon Labs/i.test(port.manufacturer)});
